...
#define N_PSQT 0
#define USE_AUXILIARY 0
#define NUM_AUXILIARY_FEATURES 0
#endif
...
struct alignas(64) nnue_input_state {
	// State data
	int16 accumulator[2][HALFKX_LAYER_SIZE];
#if (N_PSQT>0)
	int32 psqt_acc[2][N_PSQT];
#endif
...
inline int scanforward64(u64 x)
{
	...
	#else
	return ((int)__builtin_popcountll(~x & (x - 1)));
	#endif
}
//
