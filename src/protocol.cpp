...
static void init() {
   ...
   if (!Init) {
      ...
      NumberThreads=option_get_int("Threads");
...
/*	FILE *fp;
   ...
   if (false) {
      ...
      send("id name Rebel " VERSION);
      send("id author Fabien Letouzey, Thomas Gaksch, Jerry Donald Watson, Chris Whittington and Ed Schroder");
...
static void parse_setoption(char string[]) {
   ...
   if (Init && my_string_equal(name,"Threads")) { // Init => already started
     
     ASSERT(!Searching);
     
     if (option_get_int("Threads")!= NumberThreads) {
       exit_threads();
       pawn_free();
       material_free();
       NumberThreads=option_get_int("Threads");
