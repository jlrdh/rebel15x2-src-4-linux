...
#define NNUE_EVAL	1		       // 0=HCE | 1=NNUE @ed

static int nnue_tuner = 100;       // @ed

// static int eval_score[256];     // @ed lazy eval doesn't work at all with NNUE
...
// 0, 0, 128, 192, 256, 320, 384, 448, 512, 576, 640, 704 ,768, 832, 896, 960,
};
...
void eval_parameter() {
   ...
   nnue_tuner = option_get_int("NNUE Tuner");
...
#if NNUE_EVAL		// 0=HCE | 1=NNUE	@ed
int eval(board_t * board, int alpha, int beta, int ThreadId, int height) {
#else
...
#if NNUE_EVAL==1		// 0=HCE | 1=NNUE	@ed
// Chris   Fruit
// nply  = height
// Color = ColourNb
// board = board

// lazy_eval = eval_score[height-1]*-1;
// lazy_eval += 500;
// eval_score[height] = lazy_eval;
// if (lazy_eval <= alpha) return (lazy_eval);
// if (lazy_eval -500 >= beta)  return (lazy_eval);

   POSITIONDATA* pos = &posdata[ThreadId][height];
   pos->eval_stack.state.valid[0] = false;
   pos->eval_stack.state.valid[1] = false;

   int us = board->turn;
   int nnue_eval = ((int)NnueEvaluate(pos, us, height));

   nnue_eval = (nnue_eval * nnue_tuner) / 100;

// eval_score[height] = nnue_eval;    // @ed
