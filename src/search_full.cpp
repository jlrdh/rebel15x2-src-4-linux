...
void Nnue_Housekeeping(board_t* board, int move, undo_t* undo, int nply, int ThreadId);
void Init_Nnue_Root(board_t* board, int nply, int ThreadId);

#define ABS(x) ((x)<0?-(x):(x))
#define MIN(X, Y)  ((X) < (Y) ? (X) : (Y))

#define CHECKS            0     // @ed | 0=0ff 1=on
#define LMR    	          1     // @ed | 0=0ff 1=on
#define IIR               1     // @ed | 0=0ff 1=on
#define PVS_EXTENSION     0		// @ed | 0=off 1=on
#define EVAL_PRUNING      0     // @ed | 0=off 1=on

static int tt_move[HeightMax];	    // @ed
static int tt_nb  [HeightMax];	    // @ed
...
//   earlyTransPruningMargin = option_get_int("Hash Pruning Margin");
   ...
   for (x = 0; x <= 63; x++)
   { for (y = 0; y <= 63; y++) {
     REDU = (int)(0.5 + log(x) * log(y) / 2.1); if (REDU < 0) REDU = 0;  QuietMoveReduction[x][y] = REDU;
     REDU--; if (REDU < 0) REDU = 0;  QuietPVMoveReduction[x][y] = REDU; } }
   #endif
...
//   int nply = 0;
//   Init_Nnue_Root(board, nply, ThreadId);       // @chris
  
   Init_Nnue_Root(board, height, ThreadId);       // @ed
   ...
   for (i = 0; i < LIST_SIZE(list); i++) {
      ...
      Nnue_Housekeeping(board, move, undo, height, ThreadId); // @Chris
...
// __builtin_prefetch(&Trans);   // ed
		  	...
		  	if (node_type != NodePV /*|| ThreadId > 0*/) {
            	...
            	if (UseMateValues) {
            		...
            		if (trans_depth < depth) {
				    	...
				    	if (!SearchCurrent[ThreadId]->trans_reduction && NumberThreads == 0){ // hash pruning doesn't work well with shared hash
				   			if (trans_depth+earlyTransPruningDepth >= depth){                 // @ed hash pruning is always bad, hence the lazy fix
   ...
   if (height >= HeightMax-1) return eval(board, alpha, beta, ThreadId, height);
   ...
   if (UseEval && node_type != NodePV && !in_check && depth <= EvalDepth){
   	    ...
   	    if (!(trans_move != MoveNone && !TRANS_IS_LOWER(trans_flags) && trans_value < beta)){
			...
			value = eval(board,threshold-1, threshold, ThreadId, height);
   ...
   if (!in_check                                  // @ed
       ...
       && node_type != NodePV
       && depth <= 6) {
       int sc = eval(board, alpha, beta, ThreadId, height) - 90 * depth;
   ...
   // if hash table tells us we are < beta, the null move search probably wastes time
      ...
      if (!in_check
       && !value_is_mate(beta)
       && do_null(board)
       && (!UseNullEval || depth <= NullReduction+1 || eval(board,alpha, beta, ThreadId, height) >= beta)) {

         // null-move search
         
		 new_depth = depth - NullReduction - depth/4; // JD: from Stockfish, 10 elo better		
         Nnue_Housekeeping(board, 0, undo, height, ThreadId); // @Chris
   ...
   if (UseRazor && node_type != NodePV && !in_check && trans_move == MoveNone && depth <= RazorDepth){ 
        ...
        if (eval(board,threshold-1, threshold, ThreadId, height) < threshold){
...
static int full_quiescence(board_t * board, int alpha, int beta, int depth, int height, mv_t pv[], int ThreadId) {
   ...
   if (!in_check) {
      ...
      value = eval(board, alpha, beta, ThreadId, height);
...
static int full_new_depth(int depth, int move, board_t * board, bool single_reply, bool in_pv, int height, bool extended, bool * cap_extended, int ThreadId) {
   ...
   *cap_extended = false; // not used currently
   ...
   #if PVS_EXTENSION==1
   if (height >=5 && tt_move[height]   && tt_nb[height]==0
                  && tt_move[height-1] && tt_nb[height-1]==0
			      && tt_move[height-2] && tt_nb[height-2]==0
				  && tt_move[height-3] && tt_nb[height-3]==0) {
				  	tt_nb[height-3]++; tt_nb[height-2]++; tt_nb[height-1]++; tt_nb[height]++; // block sequence to avoid search explosion
				    return new_depth+1; } // @ed
   #endif
   ...
   #if CHECKS==1
    if (move_is_check(move,board) && (in_pv || see_move(move,board) >= -100)) return new_depth+1;
   #endif
